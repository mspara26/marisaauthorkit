...
function extractHeader(row) {
  // Header row has h2 (not h3) and no image in first column
  ...
  header.classList.add('carousel-product-header');

  // Extract rubric (first p before h2)
  const allChildren = [...cell.children];
  const h2Index = allChildren.indexOf(h2);

  allChildren.forEach((child, idx) => {
    if (idx < h2Index && child.tagName === 'P' && !child.querySelector('a')) {
      const rubric = document.createElement('div');
      rubric.classList.add('carousel-product-rubric');
      rubric.textContent = child.textContent;
      header.append(rubric);
    } else if (child.tagName === 'H2') {
      header.append(child);
    } else if (child.tagName === 'P' && !child.querySelector('a')) {
      const desc = document.createElement('p');
      desc.classList.add('carousel-product-description');
      desc.textContent = child.textContent;
      header.append(desc);
    } else if (child.tagName === 'P' && child.querySelector('a')) {
      const cta = document.createElement('div');
      cta.classList.add('carousel-product-cta');
...
export default async function decorate(block) {
  ...
  let headerEl = null;
  let slideRows = rows;
  if (rows.length > 0 && rows[0].querySelector('h2')) {
    headerEl = extractHeader(rows[0]);
    rows[0].remove();
  ...
  slideRows.forEach((row, idx) => {
    ...
    if (slideIndicators) {
      ...
      indicator.innerHTML = `<button type="button" aria-label="Show Slide ${idx + 1} of ${slideRows.length}"></button>`;
  ...
  // Build layout: header left, slides right
  if (headerEl) {
    block.prepend(headerEl);
  }
